...
import { useQuotes, type QuoteItemInput } from '@/hooks/useQuotes'
import { useCustomers } from '@/hooks/useCustomers'
import { useProducts } from '@/hooks/useProducts'
import { formatCurrency } from '@/lib/utils'
import type { QuoteStatus, TaxType } from '@/types'
...
export default function QuoteForm() {
  ...
  const updateItem = useCallback(
    (index: number, updates: Partial<QuoteItemInput>) => {
      setItems((prev) =>
        prev.map((item, i) => {
  ...
  const handleSave = async (status: QuoteStatus) => {
    ...
    let result
    if (isEdit && id) {
      result = await updateQuote(id, input, validItems)
    } else {
      result = await createQuote(input, validItems)
    }
